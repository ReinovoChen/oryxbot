...
std::vector<double> OryxBotKinematics::inverse_kinematics(double vx, double vy, double vth)
{
    ...
    case 3:
     ...
     case 4:
        ...
        motor_speed[0] = -(vx + vy - vth*(m_width + m_length))/m_wheel_radius*30/PI;
        motor_speed[1] = (vx - vy + vth*(m_width + m_length))/m_wheel_radius*30/PI;
        motor_speed[2] = -(vx - vy - vth*(m_width + m_length))/m_wheel_radius*30/PI;
        motor_speed[3] = (vx + vy + vth*(m_width + m_length))/m_wheel_radius*30/PI;
...
std::vector<double> OryxBotKinematics::kinematics(std::vector<double> motorspeed)
{
    ...
    case 3:
     ...
     case 4:
        ...
        velocity[0] = (-motorspeed[0] + motorspeed[1] 
                        - motorspeed[2] + motorspeed[3])/4*PI*m_wheel_radius/30.f;
        velocity[1] = (-motorspeed[0] - motorspeed[1] 
                        + motorspeed[2] + motorspeed[3])/4*PI*m_wheel_radius/30.f;
        velocity[2] = (motorspeed[0] + motorspeed[1] 
                        + motorspeed[2] + motorspeed[3])/2*PI*m_wheel_radius/30.f
                        /(m_width + m_length);
        break;
     default:
...
{
    m_vel_sub = m_h.subscribe<geometry_msgs::Twist>("cmd_vel", 100, &OryxBotKinematics::vel_callback, this);
